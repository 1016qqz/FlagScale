...
class LazySupervisedDataset(Dataset):
    ...
    def process_video(self, video_file):
        ...
        video_processed = processor.preprocess(videos=video, do_sample_frames=False, return_tensors="pt")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] and len(sources[0]["image"]):
            ...
            for merged_thw in grid_thw_merged:
                tokens = merged_thw.prod().item() if hasattr(merged_thw.prod(), 'item') else merged_thw.prod()
        ...
        if data_dict["input_ids"].shape[0] > max_len:
            ...
            if "image" in self.list_data_dict[i]:
                ...
                expected_tokens = sum(
                    t.prod().item() // self.data_args.image_processor.merge_size ** 2 for t in grid_thw
                )
...
@dataclass
class DataCollatorForQwen35(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, mm_token_type_ids = tuple(
            [instance[key] for instance in instances] for key in ("input_ids", "labels", "mm_token_type_ids")
        )
